...
const replacements = {
    ...
    'forceLatex': [ // This category seems unused by the main exported function.
...
export function replaceInlineLatexWithCodeBlocks(text) {
    ...
    // This regex looks for various LaTeX delimiters and captures the content.
    // It tries to handle $$...$$, $...$ (avoiding things like var$iable), \[...\], and \(...\)
    return text.replace(/\\\[(.*?)\\\]|\\\((.*?)\\\)|\$\$(.*?)\$\$|(?:^|\s)\$((?:\\.|[^$])+)\$(?:\s|$)/gs,
        (match, square, round, double, single) => {
            const latex = square || round || double || single;
            if (latex) { // Ensure latex content exists
                return `\n\`\`\`latex\n${latex.trim()}\n\`\`\`\n`;
            }
            return match; // If no latex content captured, return original match
        });
}

export default function markdownToPango(text) {
    if (typeof text !== 'string') return '';
    if (typeof text !== 'string') return '';
    ...
    for (const line of lines) {
        if (codeBlockRegex.test(line)) {
            ...
            continue; // Skip fence lines from Pango output
        }

        if (inCodeBlock) {
            continue; // Skip lines within code blocks from Pango output
        }

        // Apply formatting only if not in a code block
        let result = line;
...
\`\`\`
An equation: $E = mc^2$ is a test.
Another one: $$a^2 + b^2 = c^2$$
And with brackets: \\(x^2 + y^2 = r^2\\)
`;
